...
clust_K = zeros(size(Testset,1),20);
clust_T = zeros(size(Testset,1),20);
clust_G = zeros(size(Testset,1),20);
clust_S = zeros(size(Testset,1),20);
...
for i=1:20  
    ...
    % K-means
    clust_K(:,i) = kmeans(Testset, i,'Distance','sqeuclidean','Display','final','Replicates',5);
